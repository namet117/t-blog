...
export default {
    ...
    methods: {
        ...
        handlePublish () {
            if (this.canPublish()) {
                ...
                (
                    ...
                    ? (Util.ajax.post('/article/' + this.articleId, data))
                    : (Util.ajax.post('/article', data))
                ).then((response) => {
        ...
        init () {
            ...
            Util.ajax.get('/article/' + this.articleId).then((response) => {
                this.formData.title = response.data.title;
                this.formData.slug = response.data.slug;
                this.formData.is_top = response.data.is_top == 1;
                this.formData.first_img = response.data.first_img;
                this.formData.keywords = response.data.keywords;
                this.tags_name = response.data.tag_names.join('，');

                this.simpleMDE.value(response.data.original_md);
    ...
    watch: {
        '$route' (val, oldVal) {
            if (val.name === 'article-info' && (val.params.article_id != this.articleId)) {
                this.init();
            }
        }
    }
